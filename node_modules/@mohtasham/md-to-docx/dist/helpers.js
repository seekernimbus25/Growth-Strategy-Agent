 ...
 */
export function processTable(tableData, documentType, style) {
    ...
    return new Table({
        ...
        rows: [
            new TableRow({
                tableHeader: true,
                children: tableData.headers.map((header, index) => new TableCell({
                    children: [
                        new Paragraph({
...
export async function processImage(altText, imageUrl, style) {
    try {
        ...
        if (!data || data.length === 0) {
            throw new Error(`Invalid image data: data length is ${data ? (data instanceof Uint8Array ? data.length : data.length) : 0}`);
        }
