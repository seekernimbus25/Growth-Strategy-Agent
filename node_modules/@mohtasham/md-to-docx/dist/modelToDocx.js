...
import { processHeading, processTable, processCodeBlock, processBlockquote, processComment, processImage, processParagraph, processListItem, } from "./helpers.js";
/**
 * Converts internal docx model to docx Paragraph/Table objects
 * Handles nested lists with proper level tracking
 */
export async function modelToDocx(model, style, options) {
    const children = [];
    const headings = [];
    const documentType = options.documentType || "document";
    // Track numbering sequences for nested lists
    let maxSequenceId = 0;
    function renderBlockNode(node, listLevel = 0) {
        switch (node.type) {
            case "heading": {
                ...
                const { paragraph, bookmarkId } = processHeading(headingLine, config, style, documentType);
                headings.push({
                    text: headingText,
                    level: node.level,
                    bookmarkId,
    ...
    function renderList(list, currentLevel) {
        const paragraphs = [];
        let itemNumber = 1;
    ...
    function renderListItem(item, isOrdered, level, sequenceId, itemNumber) {
        const paragraphs = [];
